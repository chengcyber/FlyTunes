...
public class FlyTunesStore extends ConsoleProgram {
	 ...
	 * the band that performs it) is already in the store.
	 * @param name
	 * @param band
	 * @return the index of the song in the store's song list and -1 otherwise.
	 */
	private int findSong(String name, String band) {
		int index = -1;
		for(int i = 0; i <songs.size(); i++) {
			if (songs.get(i).getBand().equals(band)
					&& songs.get(i).getTitle().equals(name)) {
				index = 1;
				break;
			}
		}
	...
	/**
	 * Adds a new song to the store's inventory
	 * 
	 * @return that song to the caller.
	 * If the song already exists in the store, it returns the existing song
	 * from the inventory.Otherwise it returns the new song that was just added to the inventory.
	 * The method may return null if the user decides not to enter a song.
	 * (i.e., user just presses Enter when asked for the song name.)
	 */
	 ...
	 * Notes that this price update will also affect all albums
